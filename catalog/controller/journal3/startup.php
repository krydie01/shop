...
//			}
			...
			if (!$this->model_journal3_settings->haveSkins()) {
				$this->print_error('Вы можете импортировать демонстрационный контент, следуя документации: <a href="https://docs.journal-theme.com/docs/demos/demo/" target="_blank">Импорт Демо</a>.');
			}

			// assets folder writable
			if (!is_writable($this->journal3->minifier->getAssetsPath())) {
				$this->print_error($this->journal3->minifier->getAssetsPath() . ' не доступно для записи! <br /> <br /> <br /> <b style="color: red">Проконсультируйтесь с вашим хостинг-провайдером для получения дополнительной информации.</b>');
			}
			...
			if ($this->journal3->isOC1()) {
				...
				if ($this->journal3->isOC31()) {
					...
					$this->journal3->document->addScript('catalog/view/theme/journal3/lib/jquery/jquery-3.4.1.min.js');
				}
			}
	...
	public function error() {
		...
		if (
			...
			$this->print_error('Journal3 должен быть активирован.');
		}
